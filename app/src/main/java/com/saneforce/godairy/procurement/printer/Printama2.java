...
import com.saneforce.godairy.SFA_Activity.DeviceListFragment;
import com.saneforce.godairy.SFA_Activity.Printama;
import com.saneforce.godairy.SFA_Activity.PrinterUtil;

import java.util.concurrent.ExecutorService;
import java.util.concurrent.Executors;
import java.util.concurrent.atomic.AtomicInteger;

public class Printama2 {
    ...
    private static Printama2 printama2;
    PrinterUtil2 util;
    private BluetoothDevice printer;
    private static int paperSize = 0;
    private Context mContext;

    public Printama2(Context context, int paperSizee) {
        Pref2.init(context);
        this.mContext = context;
        printer = getPrinter();
        util = new PrinterUtil2(printer, paperSizee);
    }

    public Printama2(Context context, String printerName) {
        Pref2.init(context);
        mContext = context;
        printer = getPrinter(printerName);
        util = new PrinterUtil2(printer, paperSize);
    }

    public static Printama2 with(Context context, Callback callback, int size) {
        Printama2 printama = new Printama2(context, size);

        callback.printama(printama);
        return printama;
    }

    public static Printama2 with(Context context, int paperSize) {
        printama2 = new Printama2(context, paperSize);
        return printama2;
    }

    static Printama2 with(Context context, String printerName) {
        printama2 = new Printama2(context, printerName);
        return printama2;
    }

    private static BluetoothDevice getPrinter() {
        return getPrinter(Pref2.getString(Pref2.SAVED_DEVICE));
    }
    ...
    private static BluetoothDevice getPrinter(String printerName) {
        ...
        for (BluetoothDevice device : defaultAdapter.getBondedDevices()) {
            //   printUsrLog("hy4yt",""+device.getAddress()+"jy"+device.getBondState()+device.getAlias()+device.getType()+device.getBluetoothClass());
...
//    }
    ...
    public static void showPrinterList(FragmentActivity activity, int activeColor, int size, OnConnectPrinter onConnectPrinter) {
        showPrinterList(activity, activeColor, size, 0, onConnectPrinter);

    }

    @SuppressLint("MissingPermission")
    public static void showPrinterList(FragmentActivity activity, int activeColor, int inactiveColor, String ex, OnConnectPrinter onConnectPrinter) {
        Pref2.init(activity);
        BluetoothAdapter defaultAdapter = BluetoothAdapter.getDefaultAdapter();
        ...
        if (defaultAdapter != null && !defaultAdapter.getBondedDevices().isEmpty()) {
            ...
            DeviceListFragment2 fragment = DeviceListFragment2.newInstance();
    ...
    @SuppressLint("MissingPermission")
    public static void showPrinterList(FragmentActivity activity, int activeColor, int size, int exint, OnConnectPrinter onConnectPrinter) {
        Pref2.init(activity);
        int inactiveColor = 0;
        BluetoothAdapter defaultAdapter = BluetoothAdapter.getDefaultAdapter();
        int activeColorResource = activeColor == 0 ? activeColor : ContextCompat.getColor(activity, activeColor);
        ...
        if (defaultAdapter != null && !defaultAdapter.getBondedDevices().isEmpty()) {
            ...
            DeviceListFragment2 fragment = DeviceListFragment2.newInstance();
            fragment.setDeviceList(defaultAdapter.getBondedDevices());
            fragment.setPaperSize(size);
            fragment.setOnConnectPrinter(onConnectPrinter);

            fragment.setColorTheme(activeColorResource, inactiveColorResource);
            fragment.show(fm, "DeviceListFragment");
            //printama  = new Printama(activity);
            // printama.paperSize=size;
            PrinterUtil2 util = new PrinterUtil2(getPrinter(),size);
            //Log.d("ytugym","papersize"+printama.paperSize);
        } else {
            onConnectPrinter.onConnectPrinter("failed to connect printer");
        }
    }
    ...
    public static void showPrinterList(Activity activity) {
        Pref2.init(activity);
        Intent intent = new Intent(activity, ChoosePrinterActivity2.class);
    ...
    private void loadBitmapAndPrint(View view, int viewWidth, int viewHeight) {
        ...
        executorService.execute(() -> printama2.printImage(b));
    }
    ...
    public interface OnConnected {
        void onConnected(Printama2 printama);
    }
    ...
    public interface Callback {
        void printama(Printama2 printama);
    }
}
